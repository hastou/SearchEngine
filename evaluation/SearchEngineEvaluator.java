...
public class SearchEngineEvaluator {
    ...
    private Vector<RecallPrecisionPoint> interpolate(Vector<RecallPrecisionPoint> points) {
        TreeMap<Double, RecallPrecisionPoint> interpolatedPoints = new TreeMap<Double, RecallPrecisionPoint>();
        double maxPrecision;
...
//        int queryId = Integer.parseInt(args[2]);
        ...
        // Initialize various type of vocabular
        Vocabular vocabular = new Vocabular(tokenizer, stopList, stemmer);
        Vocabular vocabularNoStemmer = new Vocabular(tokenizer, stopList);
        Vocabular vocabularNoStemmerNoStopList = new Vocabular(tokenizer);


        Map<String, SearchEngine> searchEngines = new TreeMap<>();

        // Initialize various types of SearchEngines
        searchEngines.put("dice_all", new SearchEngineDice(vocabular));
        searchEngines.put("dice_no_stemmer", new SearchEngineDice(vocabularNoStemmer));
        searchEngines.put("dice_no_stemmer_no_stoplist", new SearchEngineDice(vocabularNoStemmerNoStopList));
        searchEngines.put("tfidf_all", new SearchEngineTFIDF(vocabular));
        searchEngines.put("tfidf_no_stemmer", new SearchEngineTFIDF(vocabularNoStemmer));
        searchEngines.put("tfidf_no_stemmer_no_stoplist", new SearchEngineTFIDF(vocabularNoStemmerNoStopList));

        try {
            // Begin to write csv
            PrintWriter printWriter = new PrintWriter(new FileOutputStream("evaluation.csv", false));
            printWriter.write("Rappel, Précision, Query ID, SearchEngines\n");

            // Iterate over various searchEngines configuration
            for (String seType : searchEngines.keySet()) {
                double map = 0;
                int i = 0;
                SearchEngine se = searchEngines.get(seType);
                // load database file
                se.loadDatabaseFile(databaseFilePath);
                // Iterate over all med sample queries
                for (int queryId = 1; queryId < 65; queryId++) {
                    ...
                    if (rpps != null) {
                        for (RecallPrecisionPoint rpp : rpps) {
                            ...
                            // write recall precision point values to csv
                            printWriter.write(rpp.toCSVLine() + "," + queryId + "," + seType + "\n");
                        }

                        // compute map if ap is not NaN
                        if (!Double.isNaN(ap)) {
                            map += ap;
                            i++;
                        }
...
//                    System.out.println("Average precision: " + ap);
                ...
                System.out.println("MAP : " + seType + " : " + map/i);
            }
            printWriter.close();
        } catch (IOException e) {
            e.printStackTrace();
